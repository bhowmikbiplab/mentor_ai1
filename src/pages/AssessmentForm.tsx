...
import {
  ArrowRight, ArrowLeft, Check, Sparkles, X, Plus,
} from 'lucide-react';
import { Navbar } from '@/components/Navbar';
import { DynamicIcon } from '@/components/DynamicIcon';
import {
  INTERESTS, SKILL_LEVELS, TECH_OPTIONS,
  type AssessmentInput, type Interest, type SkillLevel,
} from '@/data/types';
...
export function AssessmentForm({ theme, setTheme, onHome, onSubmit }: AssessmentFormProps) {
  ...
  const handleNext = () => {
    if (step < 2) setStep(step + 1);
      ...
      onSubmit({ interests, skillLevel, techStack });
    }
  };
  ...
  return (
    <div className="min-h-screen">
      <Navbar theme={theme} setTheme={setTheme} onHome={onHome} showBack onBack={handleBack} />
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-10">
            ...
            <div className="flex justify-between px-0">
              {STEPS.map((label, i) => (
                <div key={label} className={`text-xs sm:text-sm font-medium ${i === step ? 'text-ink-700 dark:text-ink-200' : 'text-ink-400 dark:text-ink-500'}`}>
                  {label}
                </div>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 0 && (
                ...
                >
                  ...
                  <p className="text-ink-500 dark:text-ink-400 mb-6">
                    Pick one or more areas you are passionate about. This shapes the project domain.
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className={`h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 ${selected ? 'bg-gradient-to-br from-accent-400 to-neon-teal' : 'bg-ink-200/50 dark:bg-ink-700/40'}`}>
                              <DynamicIcon name={interest.icon} className={`h-5 w-5 ${selected ? 'text-white' : 'text-ink-500 dark:text-ink-300'}`} />
                            </div>
                            <div>
                              <div className="font-semibold text-ink-900 dark:text-white">{interest.label}</div>
                              <div className="text-sm text-ink-500 dark:text-ink-400 mt-0.5">{interest.blurb}</div>
                            </div>
                          </div>
                          {selected && (
                            <motion.div
              ...
              {step === 1 && (
                ...
                >
                  ...
                  <p className="text-ink-500 dark:text-ink-400 mb-6">
                    Be honest — this determines project difficulty and scope.
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className={`h-12 w-12 rounded-xl flex items-center justify-center flex-shrink-0 ${selected ? 'bg-gradient-to-br from-accent-400 to-neon-teal' : 'bg-ink-200/50 dark:bg-ink-700/40'}`}>
                              <DynamicIcon name={level.icon} className={`h-6 w-6 ${selected ? 'text-white' : 'text-ink-500 dark:text-ink-300'}`} />
                            </div>
                            <div className="flex-1">
                              <div className="font-semibold text-lg text-ink-900 dark:text-white">{level.label}</div>
                              <div className="text-sm text-ink-500 dark:text-ink-400 mt-0.5">{level.blurb}</div>
                            </div>
                            {selected && (
                              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="h-7 w-7 rounded-full bg-accent-500 flex items-center justify-center">
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <p className="text-ink-500 dark:text-ink-400 mb-6">
                    Select the technologies you are comfortable with. You can add your own.
                  </p>
                      ...
                      {techStack.map((tech) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="h-5 w-5 rounded-md hover:bg-accent-400/20 flex items-center justify-center"
                          >
                  ...
                  <div className="flex gap-2 mb-5">
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl glass hover:shadow-glow transition-all flex items-center gap-1.5 text-sm font-medium text-ink-700 dark:text-ink-200"
                    >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {TECH_OPTIONS.filter((t) => !techStack.includes(t)).map((tech) => (
                      <button
                        key={tech}
                        onClick={() => toggleTech(tech)}
                        className="px-3.5 py-2 rounded-lg glass hover:bg-accent-400/10 hover:border-accent-400/40 text-sm font-medium text-ink-600 dark:text-ink-300 transition-all hover:scale-105"
                      >
                        + {tech}
                      </button>
                    ))}
